...
function buildParams(
	...
	if (compat.thinkingFormat === "zai" && model.reasoning) {
		...
		if (options?.reasoningEffort && compat.supportsReasoningEffort && isModelStudioModel(model)) {
			const effort =
				options.reasoningEffort === "minimal" && model.id.startsWith("deepseek-v4-")
					? "low"
					: options.reasoningEffort;
			Object.assign(params, {
				reasoning_effort: model.thinkingLevelMap?.[options.reasoningEffort] ?? effort,
			});
		}
		...
		const openRouterParams = params as typeof params & { reasoning?: { effort?: string; enabled?: boolean } };
		if (options?.reasoningEffort) {
			openRouterParams.reasoning = {
				effort: model.thinkingLevelMap?.[options.reasoningEffort] ?? options.reasoningEffort,
			};
		} else if (model.thinkingLevelMap?.off !== null) {
			const offValue = model.thinkingLevelMap?.off ?? "none";
			openRouterParams.reasoning = offValue === "none" ? { enabled: false } : { effort: offValue };
		}
