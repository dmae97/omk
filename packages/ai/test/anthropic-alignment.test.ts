...
describe("Anthropic request fingerprint alignment", () => {
	...
	it("gates the effort beta and field off google-vertex requests (#5614)", async () => {
		...
		await streamAnthropic(vertexModel, { systemPrompt: ["Stay concise."], messages: [{ role: "user", content: "Hi", timestamp: Date.now() }] }, { apiKey: "vertex-adc", thinkingEnabled: true, effort: "high", fetch: fetchMock, fallbacks: [
					{
