...
describe("Neo distribution and safe setup", () => {
	it("discovers six real bundled skills in a clean directory without writing MCP config", () => {
		const loaded = loadSkillsWithBundled(false, {
			cwd: root, agentDir: join(root, "agent"), skillPaths: [], includeDefaults: false,
		});
	...
	it("connects bundled discovery to the real DefaultResourceLoader", async () => {
		...
		await loader.reload();
		expect(loader.getSkills().skills.map((skill) => skill.name).sort()).toEqual([...NEO_SKILL_NAMES].sort());
	});
	it("does not falsely report MCP connections or perform setup during listing", () => {
		const output: string[] = [];
		expect(runNeoCli(["list"], context((text) => output.push(text)))).toBe(0);
	...
	it("previews setup without writing and emits disabled config by default", () => {
		const output: string[] = [];
		expect(runNeoCli(["setup", "playwright"], context((text) => output.push(text)))).toBe(0);
	...
	it("creates only the selected enabled stdio entries after explicit apply", () => {
		const output: string[] = [];
		expect(runNeoCli(["setup", "playwright", "context7", "--apply"], context((text) => output.push(text)))).toBe(0);
	...
	it("never overwrites or exposes an existing configuration, even if malformed", () => {
		...
		const output: string[] = [];
		expect(runNeoCli(["setup", "playwright", "--apply"], context((text) => output.push(text)))).toBe(1);
