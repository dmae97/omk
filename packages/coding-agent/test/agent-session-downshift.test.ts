...
describe("AgentSession downshift", () => {
	...
	it("boomerang hands back to the original model at the terminal yield, with the fast-model era cut from context", async () => {
		const primary = modelOrThrow("claude-sonnet-4-5");
		const target = modelOrThrow("claude-sonnet-4-6");
		const modelRegistry = new ModelRegistry(authStorage, path.join(tempDir.path(), "models.yml"));
		const planMarker = "complete plan in your NEXT reply";
		const checklistMarker = "grep for every other call site";
		const boomerangMarker = "working state is uncertain";

		// Turn 1: explore (nudge injected after). Turn 2: write — switch, boomerang
		// armed. Turn 3 (fast model): text-only yield with no incomplete todos —
		// boomerang fires: context cut back to the pre-switch state plus the
		// hand-back message, model returns to primary. Turn 4 (primary): validates.
		const mock = createMockModel({
			responses: [
				toolCall("t1", "record"),
				toolCall("t2", "write"),
				{ content: [{ type: "text", text: "All done — implemented the fix." }], stopReason: "stop" },
				{ content: ["validated, complete"] },
			],
		});
		const calls: Array<{ model: string; hasChecklist: boolean; hasBoomerang: boolean; hasFlashReport: boolean }> = [];
		const agent = new Agent({
			getApiKey: () => "test-key",
			initialState: {
				model: primary,
			...
			streamFn: (model, context, options) => {
				calls.push({
					model: `${model.provider}/${model.id}`,
					hasChecklist: contextMessagesHaveMarker(context.messages, checklistMarker),
					hasBoomerang: contextMessagesHaveMarker(context.messages, boomerangMarker),
					hasFlashReport: contextMessagesHaveMarker(context.messages, "All done — implemented the fix."),
				});
				return mock.stream(model, context, options);
			},
		});
		...
		// Checklist visible only during the fast-model era; cut at the hand-back.
		expect(calls.map(call => call.hasChecklist)).toEqual([false, false, true, false]);
		// Hand-back message present only on the validation turn, carrying the
		// fast model's final report.
		expect(calls.map(call => call.hasBoomerang)).toEqual([false, false, false, true]);
		expect(calls[3].hasFlashReport).toBe(true);
		// Back on the original model at the end, and the plan nudge stays scrubbed.
		expect(session.model?.id).toBe(primary.id);
		expect(contextMessagesHaveMarker(agent.state.messages, planMarker)).toBe(false);
	});
});
