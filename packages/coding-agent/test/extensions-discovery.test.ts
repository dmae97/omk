...
describe("extensions discovery", () => {
	...
	const extensionCode = `
		export default function(omk) {
			omk.registerCommand("test", { handler: async () => {} });
		}
	`;
	...
	it("registers message renderers", async () => {
		const extCode = `
			export default function(omk) {
				omk.registerMessageRenderer("my-custom-type", (message, options, theme) => {
	...
	it("reports error when extension has no default export", async () => {
		const extCode = `
			export function notDefault(omk) {
				omk.registerCommand("test", { handler: async () => {} });
			}
		`;
	...
	it("loads extension with event handlers", async () => {
		const extCode = `
			export default function(omk) {
				...
				omk.on("tool_call", async (event) => undefined);
