...
function createTempFile(): string {
	const dir = mkdtempSync(join(tmpdir(), "omk-format-resume-command-"));
	tempDirs.push(dir);
...
function createSessionManager(options: {
	...
	return {
		...
		getSessionDir: () => options.sessionDir ?? "/tmp/omk-sessions",
...
describe("formatResumeCommand", () => {
	...
	it("includes unquoted safe session dirs for non-default session dirs", () => {
		...
		const sessionManager = createSessionManager({
			...
			sessionDir: "/tmp/custom-omk-sessions",
			usesDefaultSessionDir: false,
		});

		expect(formatResumeCommand(sessionManager)).toBe(
			`${APP_NAME} --session-dir /tmp/custom-omk-sessions --session test-session`,
		);
	});
	...
	it("quotes session dirs containing single quotes", () => {
		...
		const sessionManager = createSessionManager({
			sessionFile,
			sessionId: "test-session",
			sessionDir: "/tmp/custom omk's sessions",
			usesDefaultSessionDir: false,
		});

		expect(formatResumeCommand(sessionManager)).toBe(
			`${APP_NAME} --session-dir '/tmp/custom omk'\\''s sessions' --session test-session`,
		);
	});
	...
	it("returns undefined when the session file is missing", () => {
		setStdoutIsTTY(true);
		const sessionManager = createSessionManager({ sessionFile: "/tmp/omk-missing-session.jsonl" });
