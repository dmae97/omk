...
export function runNeoCli(args: readonly string[], context: NeoCliContext): number {
	...
	try {
		...
		if (command === "setup") {
			if (rest.filter((arg) => arg === "--apply").length > 1 || rest.filter((arg) => arg === "--global").length > 1) {
			...
			if (!apply) {
				context.output(JSON.stringify({
					status: "dry_run", target: join(root, ".omk", "mcp.json"),
					presets, config: neoMcpConfig(ids, false),
					approval: "--apply creates enabled entries. On the next OMK startup npx may download and execute the pinned servers, which can access the network. No server is started by this command.",
				}, null, 2));
				return 0;
			}
			const target = createNeoMcpConfig(root, ids);
			context.output(JSON.stringify({
				status: "configured_not_connected", target, presets: ids,
		...
		context.output(JSON.stringify({ status: "blocked", error: error instanceof Error ? error.message : "Neo command failed" }));
		return 1;
	}
}
