...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, undefined/false if omk-generated */
	fromHook?: boolean;
}
...
export interface BranchSummaryEntry<T = unknown> extends SessionEntryBase {
	...
	details?: T;
	/** True if generated by an extension, false if omk-generated */
	fromHook?: boolean;
}
...
async function buildSessionInfo(filePath: string): Promise<SessionInfo | null> {
	try {
		...
		let messageCount = 0;
		let firstMessage = "";
		const allMessagesDigest = new BoundedDigestAccumulator({ maxChars: DEFAULT_SESSION_DIGEST_MAX_CHARS });
		...
		for await (const line of rl) {
			...
			allMessagesDigest.push(textContent);
			if (!firstMessage && message.role === "user") {
				firstMessage = boundSessionFirstMessage(textContent);
			}
		}
		...
		return {
			...
			allMessagesText: allMessagesDigest.result().text,
		};
	} catch {
		return null;
	}
