...
export function createNeoMcpConfig(root: string, ids: readonly string[]): string {
	...
	if (!info.isDirectory() || info.isSymbolicLink())
		throw new Error("Neo setup refuses a symlinked or non-directory .omk");
	...
	try {
		...
		if (code === "EEXIST")
			throw new Error("mcp.json already exists; no changes made. Use neo mcp-config and review a manual merge.");
