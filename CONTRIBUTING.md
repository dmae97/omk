...
Docs checklist for feature PRs: update the matching `packages/coding-agent/docs/*.md` in the same PR; release PRs edit only `CHANGELOG.md` — the root `README.md` release block is generated by `scripts/sync-readme-releases.mjs`, never edit it by hand; `npm run check` enforces both via `check:readme-releases` and `check:doc-links`.
