...
> **상태:** 2026-07-19 로컬 freeze → 2026-07-23 published. workspace 패키지 버전은 현재 `0.91.0`이며, `0.90.9`는 공개 릴리스로 발행되었다. 이 문서는 발행된 0.90.9의 계획 기록이다.
...
> **작성일:** 2026-07-15
>
> **로컬 freeze 스냅샷 (2026-07-19, published 2026-07-23):** closed tool turn, deterministic resource-claim DAG와 `waves-v1` rollback, execution-bound evidence, transactional compaction, typed termination/session doctor, provider-origin-aware doctor 구현 표면을 문서화했다. `0.90.9`는 공개 릴리스로 발행되었으며, 현재 workspace는 `0.91.0` 라인이다.
>
...
v0.90.9는 공개 릴리스로 발행되었으며(2026-07-23), 기능 확장보다 런타임 신뢰성 강화에 집중한 lockstep workspace snapshot이었다. 현재 트리는 `0.91.0` 라인이다.
...
**릴리스 문장:** OMK v0.90.9 closes every tool turn, schedules independent work through a deterministic resource DAG, binds evidence to fresh execution receipts, and makes session termination diagnosable and recoverable.
...
| 7 | Release documentation | changelog, release note, README, upgrade plan | implemented-surface summary, migration/rollback boundary, verification boundary | `0.90.9` published; workspace advanced to `0.91.0` |
