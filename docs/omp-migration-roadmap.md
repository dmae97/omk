...
> **Current verdict — source import `ACCEPTED`; G1/H1 `PASS`; G2 `ACCEPTED (scope-bounded)`; I1 `COMPLETE`; G3 `PASS`; R1 `DECIDED (publication disabled)`:** [ADR-OMP-007](adr/ADR-OMP-007.md) records the operator-accepted G3 verification (all seven gates PASS on fresh 2026-07-21 evidence, zero OMP-attributable regression, byte-exact rehearsed rollback), acknowledges the owner-checkout main state as-is with no publication (local `main` is 13,716 commits ahead of `origin/main`; nothing pushed), and decides R1: publication, release, tags, branch pushes, and the fork PR route remain disabled. Reopening publication requires a fresh explicit operator decision.
...
- **Controlling decisions:** [ADR-OMP-001](adr/ADR-OMP-001.md), [ADR-OMP-002](adr/ADR-OMP-002.md), [ADR-OMP-003](adr/ADR-OMP-003.md), [ADR-OMP-005](adr/ADR-OMP-005.md), [ADR-OMP-006](adr/ADR-OMP-006.md), and [ADR-OMP-007](adr/ADR-OMP-007.md)
