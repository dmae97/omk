...
> **Current verdict — source import `ACCEPTED`; G1/H1 `PASS`; G2 `ACCEPTED (scope-bounded)`; I1 `COMPLETE`; G3 `PASS`; R1 `DECIDED (publication disabled)`; I2 `COMPLETE (flag-gated tool wiring)`:** [ADR-OMP-008](adr/ADR-OMP-008.md) wires the `read`/`grep` tools to the OMP pure seams behind `OMK_OMP_SEAMS=1` (default off; flag-off behavior byte-identical), records full-suite regression parity with the pre-I2 baseline, and records the benchmark verdict: the OMP seam is **not faster** (read 1.07→88.96 ms mean for 2,001 provenance digests; grep +16%) — its value is deterministic source-bound presentation, not speed. [ADR-OMP-007](adr/ADR-OMP-007.md) keeps publication disabled.
...
- **Controlling decisions:** [ADR-OMP-001](adr/ADR-OMP-001.md), [ADR-OMP-002](adr/ADR-OMP-002.md), [ADR-OMP-003](adr/ADR-OMP-003.md), [ADR-OMP-005](adr/ADR-OMP-005.md), [ADR-OMP-006](adr/ADR-OMP-006.md), [ADR-OMP-007](adr/ADR-OMP-007.md), and [ADR-OMP-008](adr/ADR-OMP-008.md)
...
| **I2. Wire read/grep tools to the seams (flag-gated)** | `COMPLETE` via [ADR-OMP-008](adr/ADR-OMP-008.md) ([benchmark](adr/evidence/omp-i2-benchmark.json)) | G3 PASS; operator instruction to proceed with wiring. | With `OMK_OMP_SEAMS=1`, `read` (text) and `grep` (context=0) delegate validation+presentation to the vendored seams; flag-off byte-identical; full-suite regression parity (same 18 pre-existing LLM-auth failures); benchmark: seam is not faster — value is source-bound presentation. | Revert the I2 commit; flag-off path untouched by construction. |
