...
- Mnemosyne collaborators:
  - `packages/coding-agent/src/mnemosyne/backend.ts` — local backend bootstrap, prompt injection, subagent aliasing, enqueue/clear.
  - `packages/coding-agent/src/mnemosyne/state.ts` — scoped recall/retain state and local writes.
  - `packages/coding-agent/src/mnemosyne/config.ts` — local SQLite path, bank, scoping, provider settings.
  - `packages/mnemosyne/src/core/memory.ts` — local memory runtime used by `remember(...)`.
...
## Flow
1. `MemoryRetainTool.createIf(...)` exposes the tool when `memory.backend` is either `"hindsight"` or `"mnemosyne"`.
2. `execute(...)` re-reads `memory.backend` and dispatches to the matching session state.
3. If the backend is `mnemosyne`:
   - it fetches `session.getMnemosyneSessionState()` and throws if the backend was not started;
   ...
   - writes go to the scoped retain bank selected by `packages/coding-agent/src/mnemosyne/config.ts`.
...
- Mnemosyne tool path: direct local `remember(...)` into the scoped retain bank.
...
- Mnemosyne bank scoping from `resolveBankScope(...)`:
...
- Session scope:
  ...
  - persisted Mnemosyne memories are local SQLite data;
...
- Filesystem
  ...
  - Mnemosyne: writes to local SQLite under `mnemosyne.dbPath`, defaulting beneath the agent memories directory (`mnemosyne/mnemosyne.db`) with one database file per scoped bank when needed.
- Network
  ...
  - Mnemosyne: none unless configured embedding or LLM providers make calls during extraction.
- Session state
  ...
  - Mnemosyne: writes through the session's scoped `Mnemosyne` instance, includes `session_id`, `cwd`, and optional `context`, and shares scoped resources with subagents.
- User-visible prompts / interactive UI
  - Hindsight async flush failures emit `session.emitNotice("warning", ...)`; the model is not told.
  - Mnemosyne write failures are logged by `rememberInScope(...)`; the tool response does not expose per-item failures.
- Background work / cancellation
  - Hindsight flush runs later on timer, queue-size threshold, `agent_end`, backend `enqueue(...)`, or backend `clear(...)`.
  - Mnemosyne fact/entity extraction may continue in the Mnemosyne runtime; backend `enqueue(...)` calls `flushExtractions()` before sleeping sessions.
...
- Mnemosyne retain settings:
  - `mnemosyne.retainEveryNTurns` default `4`
  - `mnemosyne.autoRetain` controls automatic retention of completed conversation turns
  - `mnemosyne.scoping` selects `global`, `per-project`, or `per-project-tagged`
...
- Mnemosyne `remember(...)` failures are caught in `MnemosyneSessionState.rememberInScope(...)`, logged, and not rethrown to the tool caller.
...
- Mnemosyne storage is local SQLite. `mnemosyneBackend.clear(...)` removes the scoped database files for the active configuration.
...
- Mnemosyne auto-retain stores prepared transcripts with `source: "coding-agent-transcript"`, `importance: 0.65`, `veracity: "unknown"`, and `memoryType: "episode"`.
