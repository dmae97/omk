...
function runSubagent(prompt: string, cwd: string): Promise<{ success: boolean }> {
	return new Promise((resolve) => {
		const child = spawn("omk", ["--mode", "json", "--tools", "read,write", "-p", prompt], {
			cwd,
		...
		child.on("error", (err) => {
			console.error(chalk.red(`  Failed to spawn omk: ${err.message}`));
...
async function main() {
	...
	const sessionsBase = join(homedir(), ".omk/agent/sessions");
	...
	const globalAgentsMd = join(homedir(), ".omk/agent/AGENTS.md");
