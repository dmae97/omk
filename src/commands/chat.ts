...
import { buildOmkToolPlaneManifest, type OmkToolPlaneDiagnostic } from "../runtime/tool-plane.js";
...
type ChatBrand = "omk" | "minimal" | "plain" | "green-rain" | "neon-grid" | "kimicat";
type ChatUi = "legacy" | "plain-modern" | "rich" | "system24" | "green-rain" | "neon-grid";
...
function renderChatIntro(
  ...
  const titleKey: Record<ChatBrand, string> = {
    omk: "chat.intro.kimicat",
    kimicat: "chat.intro.kimicat",
    minimal: "chat.intro.minimal",
    plain: "chat.intro.plain",
    "green-rain": "chat.intro.kimicat",
    "neon-grid": "chat.intro.kimicat",
  };
...
export async function chatCommand(options: {
  ...
  // Dependency preflight: Kimi binary + node-pty are required only when the
  // Kimi adapter is explicitly selected. Provider-neutral auto mode must
  // survive no-Kimi smoke and route diagnostics.
