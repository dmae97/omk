...
export { buildParallelRouteDecision } from "./parallel/orchestrator.js";
...
function buildDeepSeekPromptPrefix(
  ...
  const lines = [
    ...
    `Direct mode is read-only. For file-affecting advisory mode, propose patch strategy only; the configured authority provider owns actual edits, merge authority, and final synthesis.`,
...
export function buildDynamicNodes(input: DynamicNodeBuildInput): DagNodeDefinition[] {
  ...
  if (intent?.needsSecurityReview && !roles.includes("security")) roles.unshift("security");
  if (intent?.needsTesting && !roles.includes("tester")) roles.push("tester");
  if (intent?.needsTesting && !roles.includes("qa")) roles.push("qa");
  if (taskType === "review" && !roles.includes("reviewer")) roles.push("reviewer");
  ...
  const authorityProvider = resolveFallbackProvider(providerPolicy);
  const providerCandidatesFor = (role: string): ProviderId[] => {
    const primary = role === "coder" || providerPolicy === "auto"
      ? authorityProvider
      : resolveFallbackProvider(providerPolicy);
    return Array.from(new Set([primary, DEFAULT_AUTHORITY_PROVIDER]));
  };
    ...
    (_, index) => {
      ...
      return {
        ...
        routing: {
          ...
          ...(providerPolicy === "auto" || providerPolicy === "authority" ? {} : { assignedProvider: resolveFallbackProvider(providerPolicy) }),
          candidateProviders: providerCandidatesFor(role),
